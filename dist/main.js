...
export default class ModuleInstance extends InstanceBase {
    ...
    async queryInitialStatus() {
        try {
            ...
            const queryRouting = await this.waitForLine(/^x([1-4])AVx1\s*,\s*x([1-4])AVx2$/i, 3000);
            ...
            const queryBlink = await this.waitForLine(/^(Blink on|Blink off)$/i, 3000);
            ...
            const queryLRAUD = await this.waitForLine(/^(LRAUD on|LRAUD off)$/i, 3000);
            ...
            const queryUsbLogic = await this.waitForLine(/^(USBHostLogic follow usb|USBHostLogic follow video|manual)$/i, 3000);
            ...
            const queryUsbRoute = await this.waitForLine(/^(USBHostRoute C|USBHostRoute 1|USBHostRoute 2|USBHostRoute 3)$/i, 3000);
